...
preprints %>%
  ...
  write_csv("data/preprint_altmetrics_20190901_20200430.csv")

```

# Full altmetrics (NOTE: should check licensing agreements with Altmetric before making data publicly available)
...
tweet_ids %>%
  ...
  filter(created_at <= "2020-04-30") %>%
  # select important fields
  select(doi, user_id, tweet_id, status_url, created_at, screen_name, text, reply_to_status_id,
         reply_to_user_id, reply_to_screen_name, is_quote, is_retweet, favorite_count,
         retweet_count, quote_count, reply_count, hashtags, retweet_status_id,
         retweet_user_id, retweet_text, retweet_created_at, retweet_retweet_count, 
         followers_count, location, description) %>%
  distinct() %>%
  write_csv("data/preprint_tweets_20190901_20200430.csv")
