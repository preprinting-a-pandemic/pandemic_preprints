...
p1A + p1B + p1C + plot_layout(ncol = 1) +
  ...
  ggsave("outputs/figures/Figure_1.png", width = 10, height = 9)
...
pS1B <-journal_articles %>% 
  ...
  mutate(article_is_oa = case_when(
    ...
    T  ~ "Closed Access")) %>%
  ggplot() +
  geom_area(aes(x = created, y = cumulative_n, 
...
# Function for basic cleaning of author names - convert to lowercase, remove
# whitespace, remove periods after initials
...
preprinter_history <- preprints_all %>%
  ...
  count(author_corresponding, author_group) %>%
  mutate(preprinter_status = case_when(
    n == 1 ~ "First-time", 
    T ~ "Previous"))
