...
p2A <- preprints %>%
  ...
  scale_fill_manual(values = qualitative_hcl(2, palette = "Set2")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggsave("outputs/figures/partials/covid_non_covid_preprints.png", width = 10, height = 5)
...
p <- p2A / (p2B | p2C + plot_layout(guides = "keep")) / (p2D | p2E) / (p2F | p2G) + 
...
p3SA + p3SB + plot_layout(ncol = 1) +
  plot_annotation(caption = caption, tag_levels = "A") +
  ggsave("outputs/figures/Figure_3.png", width = 10, height = 5)
...
# SSRN
ssrn_preprints <- read.csv("data/otherservers/ssrn_extract.csv", 
                           quote = "", 
...
covid_usage <- d %>% 
  filter(posted_date >= "2020-01-01",
         covid_preprint == "COVID-19 preprints") %>%
  select(citations, twitter, news, blogs)

non_covid_usage <- d %>% 
  filter(posted_date >= "2020-01-01",
         covid_preprint == "non-COVID-19 preprints") %>%
  select(citations, twitter, news, blogs)
...
p2 <- (p4E + p4F) + 
  plot_layout(ncol = 2, guides = "collect") & theme(plot.margin = margin(0, 0, 0, 0),
