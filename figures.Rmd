...
p2E_properties <- preprints %>%
  ...
  scale_x_date(date_breaks = "1 month",
               ...
               date_labels = "%b") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggsave("outputs/figures/partials/Figure_2E_properties.png", width = 10, height = 4)

# Reference counts
p2F_properties <- preprints %>%
  filter(posted_date >= analysis_start,
         posted_date <= analysis_end) %>%
  mutate(posted_month = floor_date(posted_date, unit = "month")) %>%
  assign_covid_preprint() %>%
  ...
  scale_x_date(date_breaks = "1 month",
               ...
               date_labels = "%b") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggsave("outputs/figures/partials/Figure_2F_properties.png", width = 10, height = 4)

p <- p2A_properties + p2B_properties + p2C_properties + p2D_properties + p2E_properties + p2F_properties +
  plot_layout(ncol = 2, guides = 'collect') + 
  plot_annotation(tag_levels = "A") & theme(legend.position = 'bottom')

p + ggsave("outputs/figures/Figure_2_properties.png", width = 10, height = 10) 
...
# Panel C: Time bwteen first case and first preprint
...
all_server_dloads <- bind_rows(preprints %>%
                                 filter(posted_date >= "2020-01-01") %>%
...
p4A_main <- d %>%
  ...
  plot_main(title_y = "% of preprints cited")
p4A_inset <- d %>%
  mutate(metric = citations) %>%
  plot_inset(title_y = "Citations")
p4A <- p4A_main + 
  annotation_custom(ggplotGrob(p4A_inset), 
...
# Twitter
p4B_main <- d %>%
  mutate(is_cited = as.numeric(twitter > 0)) %>%
  plot_main(title_y = "% of preprints tweeted")
p4B_inset <- d %>%
  mutate(metric = twitter) %>%
  plot_inset(title_y = "Tweets")
p4B <- p4B_main + 
  annotation_custom(ggplotGrob(p4B_inset), 
...
p4C_main <- d %>%
  ...
  plot_main(title_y = "% of preprints mentioned in news")
p4C_inset <- d %>%
  mutate(metric = news) %>%
  plot_inset(title_y = "News")
p4C <- p4C_main + 
  annotation_custom(ggplotGrob(p4C_inset), 
                    xmin = as.Date("2020-06-01"), 
                    xmax = as.Date("2020-11-07"), 
                    ymin = 0.4, ymax = 1.04) +
...
p4D_main <- d %>%
  ...
  plot_main(title_y = "% of preprints mentioned in blogs")
p4D_inset <- d %>%
  mutate(metric = blogs) %>%
  plot_inset(title_y = "Blogs")
p4D <- p4D_main + 
  annotation_custom(ggplotGrob(p4D_inset), 
                    xmin = as.Date("2020-06-01"), 
                    xmax = as.Date("2020-11-07"), 
                    ymin = 0.4, ymax = 1.04) +
...
p4E_main <- d %>%
  ...
  plot_main(title_y = "% of preprints cited in Wikipedia")
p4E_inset <- d %>%
  mutate(metric = wikipedia) %>%
  plot_inset("Wikipedia")
p4E <- p4E_main + 
  annotation_custom(ggplotGrob(p4E_inset), 
                    xmin = as.Date("2020-06-01"), 
                    xmax = as.Date("2020-11-07"), 
                    ymin = 0.4, ymax = 1.04) +
...
p4F_main <- d %>%
  ...
  plot_main(title_y = "% of preprints commented on")
p4F_inset <- d %>%
...
covid_usage <- d %>% 
  ...
  select(citations, twitter, news, blogs, wikipedia, comments)
...
p4H <- non_covid_corr %>%
  ...
  theme(plot.title = element_text(size = 10)) +
  guides(fill = FALSE) +
  ggsave("outputs/figures/partials/Figure_4H.png", width = 5, height = 4)


# Patchwork
p <- p4A + p4B + p4C + p4D + p4E + p4F + p4G + p4H +
  plot_layout(nrow = 4, guides = 'collect') & theme(legend.position = "top")

p +  plot_annotation(tag_levels = "A") +
  ggsave("outputs/figures/Figure_4.png", width = 10, height = 13)
...
p4H <- policy_docs %>%
  ...
  mutate(preprint_prop = npreprints*100/nrefs,
         set = case_when(set == "ECDC other docs" ~ "ECDC",
                         set == "ECDC rapid risk assessment" ~ "ECDC",
                         set == "ECDC technical reports" ~ "ECDC",
                         set == "National academy of medicine" ~ "NAM",
                         set == "US committee COVID" ~ "US HSSCC",
                         set == "WHO scientific brief" ~ "WHO SB",
                         set == "UK post" ~ "UK POST")) %>%
  ...
  ggsave("outputs/figures/partials/Figure_4H.png", width = 6, height = 3)
