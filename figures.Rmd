...
covid_authors <- preprints %>% filter(covid_preprint == T) %>% pull(author_corresponding)
control_authors <- preprints %>% filter(covid_preprint == F & !(author_corresponding %in% covid_authors)) %>% pull(author_corresponding)
preprinter_history <- preprints_all %>% 
  ...
  group_by(author_corresponding, author_group) %>%
  summarise(n = n()) %>%
  mutate(preprinter_status = ifelse(n == 1, "first-timer", "previous"))
