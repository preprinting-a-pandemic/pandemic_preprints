...
other_preprints <- read_csv("https://raw.githubusercontent.com/nicholasmfraser/covid19_preprints/master/data/covid19_preprints.csv", col_types = "cccccc")
...
p1 <- preprints %>%
  filter(posted_date >= "2020-01-01",
         institution_match_country_code %in% top_countries) %>%
  ...
  ggplot(aes(x = institution_match_country_code, y = n, fill = covid_preprint),
         color = "grey50") +
  geom_col(position = "dodge") +
  labs(x = "", y = "Preprints deposited", fill = "") +
  scale_y_continuous(labels = scales::comma)
