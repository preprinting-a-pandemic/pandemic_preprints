...
# Panel D: Publishing developments

# Next time just make an image somewhere else and insert to the figure with ggdraw...this was too much

d <- tibble(
  event_date = c("2020-01-31",
                 "2020-03-13",
                 "2020-03-16",
                 "2020-03-25",
                 "2020-04-27"),
  event_name = c("Call for researchers, journals and publishers to rapidly and openly share COVID-19 findings[23]",
                 "Science leaders ask publishers to make COVID content open and re-usable[24]", "Over 30 publishers agree to make COVID-19 content freely accessible via PubMed[25]", 
                 "eLife alters peer review policies in light of COVID-19[26]", 
                 "OASPA issue open letter of intent to maximise efficiency of peer-review for COVID-19 work[27]"),
  x_position = c(0,-6,0,6,0),
  y_position = c(-1,0.8,-0.8,0.6,1),
  direction = c(-1, 1, -1, 1, 1),
  label_x_position = c(-4,4,-4,4,4)) %>%
  mutate(event_date = ymd(event_date),
         event_name = factor(event_name))

p1D <- ggplot(d) +
geom_text( aes(x = (event_date + x_position), y = (direction + y_position), label = str_wrap(event_name, 22)), size = 3) +
  geom_hline(yintercept = 0, size = 1) +
  geom_segment(aes(x = event_date, y = direction, xend = event_date, yend = 0)) +
  geom_point(aes(x = event_date, y = 0), shape = 21, fill = "grey90", color = "grey50", size = 3)  +
  geom_text(data = d, aes(x = (event_date + label_x_position), y = (direction*0.5), label = event_date), size = 2, angle = 45, color = "grey25") +
  scale_y_continuous(limits = c(-2.75, 2.75)) +
  scale_x_date(limits = c(ymd("2020-01-01"), ymd("2020-05-15"))) +
  labs(y = "", x = "") +
  theme(axis.text = element_blank(),
        plot.background = element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  ggsave("outputs/figures/partials/Figure_1D.png", width = 10, height = 5)
