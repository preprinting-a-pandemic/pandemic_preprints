...
p1A + p1B + p1C + plot_layout(ncol = 1) +
  ...
  ggsave("outputs/figures/Figure_1.png", width = 10, height = 8)
...
plot(euler(fit), 
     fills = qualitative_hcl(n = 4, palette = "Set2"),
     alpha = 0.75,
     labels = list(cex = 1),
...
# Now we fit all the PNG files together. Some manual adjustments are necessary
...
preprint_author_last_covid <- preprints %>% 
  ...
  slice(n()) %>%
  select(author_corresponding, category) %>%
...
p5E <- preprint_changes %>%
  ...
  labs(title = "Abstract change", x="", y = "% of preprints", fill = "") +
...
p5F <- preprint_change %>%
  ...
  geom_col(position = "dodge", color = "grey50", size = 0.25) +
  labs(title = "Main body change", x = "", y = "% of preprints", fill = "") +
  scale_fill_manual(values = qualitative_hcl(n = 2, palette = "Set2")) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 25)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none") +
...
# Panel I (?): Plot of individual polarity of tweets per top 10 most-tweeted COVID-19 preprints
preprint_tweets_sent <- read.csv("data/preprint_sentiment_tweets_20190901_20200430.csv") %>%
    mutate(twitter_rank = factor(twitter_rank, levels=c(1:10)))
pS4I_tweets <- preprint_tweets_sent %>%
  ggplot(aes(x = twitter_rank, y = ave_sentiment, colour = qualitative_hcl(2, palette = "Set2")[1], label=title)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(fill = qualitative_hcl(2, palette = "Set2")[1]), shape = 21, size = 1, alpha = 0.2,
              position = position_jitterdodge(jitter.width = 0.4)) + 
  labs(x = "Ranking (number of tweets)", y = "Tweet sentiment polarity score", 
       fill = "", color = "") +
  guides(color = FALSE, fill = FALSE) +
  ggsave("outputs/figures/partials/Figure_S4I.png", width = 10, height = 6)
...
# Panel E (?): Plot of individual polarity of comments per top 10 most-commented COVID-19 preprints
preprint_comments_sent <- read.csv("data/preprint_sentiment_comments_20190901_20200430.csv") %>%
  mutate(comment_rank = factor(comment_rank, levels=c(1:10)))
pS5E_comments <- preprint_comments_sent %>%
  ggplot(aes(x = comment_rank, y = ave_sentiment, colour = qualitative_hcl(2, palette = "Set2")[1], label=title)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(fill = qualitative_hcl(2, palette = "Set2")[1]), shape = 21, size = 1, alpha = 0.2,
              position = position_jitterdodge(jitter.width = 0.4)) + 
  labs(x = "Ranking (number of comments)", y = "Comment sentiment polarity score", 
       fill = "", color = "") +
  guides(color = FALSE, fill = FALSE) +
  ggsave("outputs/figures/partials/Figure_S5E.png", width = 10, height = 6)
```
...
preprint_author_last %<>% left_join(preprints_all %>% 
  ...
  ungroup
# Plot alluvial diagram
p5C <- ggplot(data = preprint_author_last %>% 
  ...
  geom_alluvium(aes(fill=category_previous), reverse=TRUE) +
  geom_stratum(reverse=TRUE, color = "grey") + 
  geom_text(size = 2.6, stat = "stratum", label.strata = TRUE, reverse=TRUE) +
  # geom_text(aes(label = freq), color="gray50", nudge_x=0.23, stat = "alluvium", reverse = TRUE) +
  # geom_text(aes(label = freq), size = 2.5, color="gray30", nudge_x=0.14, stat = "stratum", reverse = TRUE) +
  # scale_fill_manual(values = qualitative_hcl(15, palette = "Set2")) +
  guides(fill=FALSE) +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
