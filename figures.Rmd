...
covid_authors <- preprints_epi_period %>% filter(covid_preprint == T) %>% pull(author_corresponding)
control_authors <- preprints_epi_period %>% filter(covid_preprint == F & !(author_corresponding %in% covid_authors)) %>% pull(author_corresponding)

preprinter_history <- preprints_all %>% 
  filter(posted_date < "2020-01-01") %>% 
  mutate(author_corresponding = factor(author_corresponding, levels = unique(c(covid_authors, control_authors)))) %>%
  filter(!is.na(author_corresponding)) %>%
  group_by(author_corresponding, .drop = FALSE) %>%
  summarise(n = n()) %>%
  mutate(author_group = case_when(
    author_corresponding %in% covid_authors ~ "COVID-19",
    author_corresponding %in% control_authors ~ "non-COVID"),
  preprinter_status = ifelse(n == 0, "first-timer", "previous")) %>%
  select(-n)

# Append author locations, with manual corrections and calculate proportions for plot
preprinter_history_plot <- preprinter_history %>%
  left_join(preprints %>% 
              mutate(institution_match_country_name = case_when(
              ...
              select(author_corresponding, institution_match_country_name, institution_match_country_code) %>%
              distinct(),
            by = "author_corresponding") %>%
  filter(!is.na(institution_match_country_name)) %>%
  filter(author_corresponding != "? ?") %>%
  ungroup() %>%
  count(author_group, preprinter_status, institution_match_country_name) %>% 
  complete(author_group, preprinter_status, institution_match_country_name, fill = list(n = 0)) %>%
  group_by(author_group, preprinter_status) %>%
  mutate(prop = n*100 / sum(n))

countries_to_plot <- preprinter_history_plot %>% 
  filter(author_group == "non-COVID" & preprinter_status == "previous") %>% 
  arrange(-prop) %>% slice(1:10) %>% 
  pull(institution_match_country_name)

p2D <- preprinter_history_plot %>%
  filter(institution_match_country_name %in% countries_to_plot) %>%
  ggplot() +
  geom_bar(mapping = aes(x = institution_match_country_name, y = prop, fill = author_group, alpha = preprinter_status), stat = "identity", position = "dodge") + 
  coord_flip() +
  ...
  xlab("Country") +
  ylab("Percentage of senior authors") +
  guides(fill = FALSE) +
...
# Panel D: Science focus of senior author
  ...
  mutate(covid_preprint = case_when(
    ...
    published_publisher = factor(published_publisher,
                                 levels = c("AME Publishing Company",
                                            ...
                                            "Wiley"),
                                 labels = c("AME", "AAAS", "ASM", "Elsevier", "Frontiers", "MDPI", "OUP", "PLoS", "Springer", "Wiley"))) %>%
