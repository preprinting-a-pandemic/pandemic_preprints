...
# Establish top 10 most tweeted COVID-19 preprints
top10_twitter <- preprints %>% 
  filter(posted_date >= "2020-01-01" & covid_preprint == TRUE) %>% 
  left_join(preprint_altmetrics, by = c("doi", "published_doi")) %>%
  arrange(-twitter) %>%
  slice(1:10) %>%
  rownames_to_column("twitter_rank") %>%
...
tweets <- read.csv("data\\preprint_tweets_20190901_20200430.csv", stringsAsFactors = FALSE) %>%
  ...
  left_join(top10_twitter %>% select(doi, title, twitter_rank), by = "doi") %>%
...
preprint_tweets_sent <- tweets %>%
  ...
  inner_join(top10_twitter %>% select(doi, title, twitter_rank), by = "doi")
...
# Save sentiment, stripping original text and id
write.csv(preprint_tweets_sent %>% select(-text, -tweet_id), file = "data/preprint_sentiment_tweets_20190901_20200430.csv")
```
...
# Save sentiment, stripping original text and id
write.csv(preprint_comments_sent %>% select(-text, -id), file = "data/preprint_sentiment_comments_20190901_20200430.csv")
```