...
parseAbstract <- function(item) {
  ...
  labels <- xpathSApply(item, "//PubmedArticleSet/PubmedArticle/MedlineCitation/Article/Abstract/AbstractText", xmlAttrs)
  
  text <- xpathSApply(item, "//PubmedArticleSet/PubmedArticle/MedlineCitation/Article/Abstract/AbstractText", xmlValue)
  
  # if labels is empty, it is returned as a list instead of a character vector
  if(!is.list(labels)) {
    labels <- str_to_title(unname(labels))
    return(str_c(str_c(labels, ": ", text), collapse = " "))
  } else {
    return(str_c(text, collapse = " "))
  }
}

getPubMedData <- function(pmid, doi) {
  
  # see https://github.com/ropensci/rentrez/issues/100 for getting abstracts from PMIDs
  d <- entrez_fetch("pubmed", id = pmid, 
                  rettype= "xml", parsed = T)
  
  return(list(published_doi = doi,
              published_pubmed_abstract = parseAbstract(d)))
  
}

getPubMedDataSafely <- safely(getPubMedData)

pubmed_data <- map2(published_pmids$pmid,
                    published_pmids$doi,
                    getPubMedDataSafely)
...
parsePubMedData <- function(item) {
  ...
  if(length(item$result$published_doi)) {
    tibble(
      ...
      published_pubmed_abstract = if(length(item$result$published_pubmed_abstract)) item$result$published_pubmed_abstract else NA_character_)
  } else {
    tibble(
...
a <- preprints %>% 
   select(-published_pubmed_abstract) %>%
   inner_join(pubmed_abstracts, by = "published_doi") %>% 
   select(doi, published_doi, abstract, published_pubmed_abstract)
