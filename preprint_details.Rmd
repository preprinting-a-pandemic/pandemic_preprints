...
# The API-selected best match (field "chosen" = TRUE) is retained.
# If multiple institutions are API-selected, the highest scoring is retained.
# If multiple institutions are API-selected and have tied scores, matches 
# are prioritised based on method with full-phrase being first.
...
getAuthorAffiliations <- function(doi, institution) {
  ...
  if(length(content$items)) {
    ...
    bind_rows(lapply(data, function(x) unlist(x) %>% t %>% as.data.frame(stringsAsFactors = FALSE))) %>% 
      filter(chosen == "TRUE") %>%
      arrange(-as.numeric(score), factor(matching_type, levels = c("PHRASE", "HEURISTICS", "COMMON TERMS", "ACRONYM"))) %>%
      slice(1) %>%
      mutate(doi = doi) %>%
