...
map_dfr(usage_data, parseUsageData) %>%
  rename(full_text_views = full,
         abstract_views = abstract,
         pdf_downloads = pdf,
         collection_date = date) %>%
  inner_join(preprints, by = c("doi", "source")) %>%
  select(doi, collection_date, abstract_views, full_text_views, pdf_downloads) %>%
  distinct() %>%
  write_csv("data/preprint_usage_20190901_20200430.csv")
